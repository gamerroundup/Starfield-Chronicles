...
async function getApiKey(urlOverride, keyOverride) {
...
async function getModel(urlOverride, keyOverride) {
  const apiKey = await getApiKey(urlOverride, keyOverride);
  if (!apiKey) {
...
export async function generateCharacterDossier(playstyle, name, background, traits, urlOverride = null, keyOverride = null) {
  const model = await getModel(urlOverride, keyOverride);
  const prompt = `
...
export async function updateChronicle(biography, stats, lastEvents, playerInput, urlOverride = null, keyOverride = null) {
  const model = await getModel(urlOverride, keyOverride);
...
export async function generateWhatsNext(biography, stats, lastEvents, urlOverride = null, keyOverride = null) {
  const model = await getModel(urlOverride, keyOverride);
...
export async function generateDailyNews(optionalCharacterInfo = null, urlOverride = null, keyOverride = null) {
  const model = await getModel(urlOverride, keyOverride);
